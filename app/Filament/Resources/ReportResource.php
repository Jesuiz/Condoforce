...
/*                 Tables\Columns\TextColumn::make('name')->label('Incidencia')
                    ->sortable()->searchable()->description(function (Report $record) {
                        $words = str_word_count($record->description, 1);
                        $limitedWords = array_slice($words, 0, 5);
                        $description = implode(' ', $limitedWords);
                        return count($words) > 6 ? $description . '...' : $description;
                    }), */
                ...
                Tables\Columns\TextColumn::make('user.name')->label('Usuario')
                    ->sortable()->searchable()
                    ->description(fn (Report $record): string => $record->condominium->name),
